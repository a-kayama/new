...
try{
 ...
 if($_POST) {
   ...
   if(empty($message['name']) && empty($message['kana']) && empty($message['mail']) && empty($message['password'])){
   $user->add($_POST);
...
<html lang="ja">
  ...
  <div id="wrapper">
    ...
    <?php if(isset($message["kana"])) echo "<p class='red'>".$message["kana"]."</p>" ?>
    ...
    <form id="new-r" action="" method="post">
      <dl>
       ...
       <dt class="kana">フリガナ:
       <input id="kana" type="text" name="kana" value=""></dt>
